...
public:
    ...
    void serializare()
    {
        ...
        int lungimeProducator = this->producator.length();
        f.write((char*)&lungimeProducator, sizeof(lungimeProducator));
        f.write(this->producator.c_str(), lungimeProducator + 1);

        int lungimeModel = this->model.length();
        f.write((char*)&lungimeModel, sizeof(lungimeModel));
        f.write(this->model.c_str(), lungimeModel + 1);

        f.write((char*)&anFabricatie, sizeof(anFabricatie));
        f.write((char*)&pret, sizeof(pret));
        f.write((char*)&numarDotariExtra, sizeof(numarDotariExtra));

        if (this->preturiDotariExtra != nullptr)
        {
            for (int i = 0; i < this->numarDotariExtra; i++)
            {
                f.write((char*)&this->preturiDotariExtra[i], sizeof(this->preturiDotariExtra[i]));
            }
        }
        f.close();
    }
    ...
    void deserializare()
    {
        ...
        int lungimeProducator = 0;
        f.read((char*)&lungimeProducator, sizeof(lungimeProducator));
        char* buffer = new char[lungimeProducator + 1];
        f.read(buffer, lungimeProducator + 1);
        this->producator = buffer;
        delete[] buffer;

        int lungimeModel = 0;
        f.read((char*)&lungimeModel, sizeof(lungimeModel));
        buffer = new char[lungimeModel + 1];
        f.read(buffer, lungimeModel + 1);
        model = buffer;
        delete[] buffer;
        buffer = nullptr;
        ...
        f.read((char*)&numarDotariExtra, sizeof(numarDotariExtra));

        if (numarDotariExtra > 0)
        {
            if (this->preturiDotariExtra != nullptr)
            {
                delete[] preturiDotariExtra;
            }
            preturiDotariExtra = new float[numarDotariExtra];

            for (int i = 0; i < this->numarDotariExtra; i++)
            {

                f.read((char*)&this->preturiDotariExtra[i], sizeof(this->preturiDotariExtra[i]));
            }
        }
        f.close();
    }
...
int main()
{
    ...
    m4.setPreturiDotariExtra(vector, 3);
